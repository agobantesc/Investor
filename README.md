...
- **Inicio (Home):** al abrir INVERSOR aterrizas en un **resumen general** — el **patrimonio total gestionado** (todos los proyectos de todos los clientes), la **rentabilidad neta consolidada**, un **podio visual de proyectos más rentables** (top-3 con medalla, sparkline, % y β, neto de comisiones; para comparar carteras: invierte montos chicos en distintas combinaciones y mide cuál rinde mejor), las **acciones destacadas** en tres rankings —por **rentabilidad**, **Sharpe** y **α de Jensen**— calculados con los **precios reales ingresados en tus seguimientos** (no con el análisis demo) y filtrables por **diario / semanal / mensual**, los **indicadores de mercado que más se movieron** y accesos rápidos.
- **Carga de precios a nivel plataforma:** desde el Home (botón **↻ Actualizar precios**) cargas el **cierre de un día** (IPSA + cada acción con seguimiento) y se agrega como nuevo punto de valoración a **todos los proyectos de todos los clientes** que tengan esas acciones.
- **Clientes:** alta y edición con un **modal con la paleta y estilo de INVERSOR** (nombre + color de identificación). Se incluyen **3 clientes de ejemplo** con historial sintético para ver el Home poblado (se pueden eliminar). El login ofrece **"mantener sesión iniciada"** para no reingresar la contraseña en cada visita.
- **Navegación:** sidebar con **Inicio** arriba y los módulos de trabajo (**Análisis · Simulación · Inversión**); **Proyectos · Perfil · Mercados** en la barra superior; **Clientes** desde el selector de cliente (⚙ Gestionar clientes); **Pro · Roadmap** desde **Configuración (⚙)**.
